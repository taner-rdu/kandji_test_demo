...
**Credentials end up in the reports.** Since Playwright 1.52, `fill()` records its argument in the step title, so the login password appears in Allure, the HTML report and the trace. Upstream closed that as won't-fix ([#35848](https://github.com/microsoft/playwright/issues/35848)), since masking the field leaves the value in snapshots and request bodies anyway. The tenant here is a month-long trial, but on a real project these reports would go to a private artifact store rather than public Pages.
